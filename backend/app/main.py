...
async def lifespan(app: FastAPI):
    ...
    # Kick the MiniLM download / load off the event loop without blocking
    # startup. A cold cache pull through the outbound proxy takes minutes,
    # during which uvicorn would otherwise stay in "Waiting for application
    # startup" and every request would 502. `get_model` is lru_cached, so
    # the first real kb_search call simply awaits the same in-flight load.
    import asyncio
